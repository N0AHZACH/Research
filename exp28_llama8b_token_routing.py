...
CSV_FILENAME = f"exp28_llama8b_token_routing_{TIMESTAMP}.csv"
SAVE_DIR     = f"exp28_llama8b_token_routing_output_{TIMESTAMP}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Phase 4.5: OpenLLaMA-3B Token-Level Routing")
    ...
    def find_latest_checkpoint():
        dirs = glob.glob("exp28_llama8b_token_routing_output_*")
        valid_checkpoints = []
        for d in dirs:
    ...
    base_model = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=COMPUTE_DTYPE, attn_implementation=ATTN_IMPL, token=hf_token).to("cuda")
    ...
    if args.resume == "auto":
        ...
        if latest_dir:
            ...
            save_dir = latest_dir
            csv_filename = f"{latest_dir.replace('exp28_llama8b_token_routing_output_', 'exp28_llama8b_token_routing_')}.csv"
    elif args.resume and args.resume.lower() != "none":
        checkpoint_dir = args.resume
        parent_dir = os.path.dirname(checkpoint_dir)
        if parent_dir:
            save_dir = parent_dir
            csv_filename = f"{parent_dir.replace('exp28_llama8b_token_routing_output_', 'exp28_llama8b_token_routing_')}.csv"
        else:
