...
CSV_OUT     = RESEARCH_DIR / "results" / f"exp20_qwen_eval_results_{TIMESTAMP}.csv"
JSON_OUT    = RESEARCH_DIR / "results" / f"exp20_qwen_eval_summary_{TIMESTAMP}.json"
...
def main():
    ...
    if args.resume:
        import glob
        json_files = glob.glob(str(RESEARCH_DIR / "results" / "exp20_qwen_eval_summary_*.json"))
        if json_files:
    ...
    if not args.skip_base and "base_tinyllama" not in completed_variants:
