...
CSV_OUT     = RESEARCH_DIR / "results" / f"exp22_qwen7b_eval_results_{TIMESTAMP}.csv"
JSON_OUT    = RESEARCH_DIR / "results" / f"exp22_qwen7b_eval_summary_{TIMESTAMP}.json"
...
def main():
    ...
    if args.resume:
        import glob
        json_files = glob.glob(str(RESEARCH_DIR / "results" / "exp22_qwen7b_eval_summary_*.json"))
        if json_files:
