"""
exp12_large_model_pareto.py - Phase 4: 3B Model Pareto Sweep

This script applies the "Zero-Storage Parallel Sweep" architecture to the Qwen2.5-3B model.
Since a compute penalty of 5.0 completely collapsed (kept 100% layers active), 
this script tests a massive spectrum of harsh compute penalties [10.0, 25.0, 50.0, 100.0, 250.0, 500.0]
...
CSV_FILENAME = f"results/exp12_large_model_pareto_{TIMESTAMP}.csv"
PLOT_FILE    = f"exp12_large_model_pareto_{TIMESTAMP}.png"
...
# ---------------------------------------------------------------------------
def main():
    print(f"\n{'='*70}\n  EXP12 LARGE MODEL PARETO SWEEP\n{'='*70}")
    ...
    def tokenize_fn(b):
        ...
        labels = [list(ids) for ids in o["input_ids"]]
        for i, mask in enumerate(o["attention_mask"]):
    ...
    plt.title("3B Model Pareto Frontier (Parallel Turbo Sweep)", fontsize=14, fontweight="bold")
