...
# During warmup the model trains on pure CE loss only, preventing the early
# KD spike (observed: KD Loss = 1864 at step 20) from destabilising training.
KD_WARMUP_STEPS  = 50
                ...
                if global_step % EVAL_EVERY_STEPS == 0:
                    ...
                    with torch.no_grad():
                        for i, val_batch in enumerate(eval_loader):
                            ...
                            # Use hard=False (soft gates) during eval — no STE
                            # needed since we are not backpropagating.
