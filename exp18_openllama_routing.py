...
CSV_FILENAME = f"results/exp18_openllama_routing_{TIMESTAMP}.csv"
SAVE_DIR     = f"checkpoints/exp18_openllama_output_{TIMESTAMP}"
...
def main():
    ...
    def find_latest_checkpoint():
        dirs = glob.glob("exp18_openllama_output_*")
        valid_checkpoints = []
        for d in dirs:
    ...
    if args.resume == "auto":
        ...
        if latest_dir:
            ...
            save_dir = latest_dir
            csv_filename = f"{latest_dir.replace('exp18_openllama_output_', 'exp18_openllama_routing_')}.csv"
    elif args.resume and args.resume.lower() != "none":
        checkpoint_dir = args.resume
        parent_dir = os.path.dirname(checkpoint_dir)
        if parent_dir:
            save_dir = parent_dir
            csv_filename = f"{parent_dir.replace('exp18_openllama_output_', 'exp18_openllama_routing_')}.csv"
        else:
